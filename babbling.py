...
class Babbler:
    ...
    def explore(self):
        ...
        while not accepted:
            ...
            # rv = self.gmm[idx]()
            cf = self.sample(idx)
            if any(p > 1.0 or p < 0.0 for p in cf):
                continue
    ...
    def evaluate(self, cf):
        ...
        goodness = 1.5 / (1.0 + np.exp(1.0*(average_distance - 8.0))) - 0.5
        # assign weight for new gmm component
        w = goodness
        center = cf
        covariance = np.identity(self.num_params) * (0.02 * (1.0 - goodness) + 0.01)
        ...
        return average_distance, w, center, covariance
